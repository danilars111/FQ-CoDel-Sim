...
while True:
    ...
    if SPARSESIZE > 0 or SPARSESIZE == -1*8:
        break
    print "invalid input, try again!"
while True:
...
while True:
    ...
    if INTERARRIVALTIME > 0 or INTERARRIVALTIME == -1 * pow(10,-3):
        break
    print "invalid input, try again!"
...
class sparsePacketGenerator(sim.Component):
...
class bulkPacketGenerator(sim.Component):
    def process(self,pid, interTime, distribution="uniform"):
...
def printQueueDelay(detailed = False):
    ...
    for x in range(len(passiveQueues)):
        if int(passiveQueues[x].packetCounter) == 0:
            print "| ", str(passiveQueues[x].qid).ljust(6), "Did not activate".ljust(26), " | "
            continue
        ...
        if detailed:
            ...
            print " ", str(round((passiveQueues[x].totalQueueDelay / passiveQueues[x].packetCounter) * 1000, 4)).ljust(14), " |"
            print "+----------+--------------+-------------------+"
    ...
    for x in range(len(newQueues)):
        if detailed:
            ...
            print " ", str(round((newQueues[x].totalQueueDelay / newQueues[x].packetCounter) * 1000, 4)).ljust(14), " |"
            print "+----------+--------------+-------------------+"
    ...
    for x in range(len(oldQueues)):
        if detailed:
            ...
            print " ", str(round((oldQueues[x].totalQueueDelay / oldQueues[x].packetCounter) * 1000, 4)).ljust(14), " |"
            print "+----------+--------------+-------------------+"
...
gen = flowGenerator(waitTime=time)
